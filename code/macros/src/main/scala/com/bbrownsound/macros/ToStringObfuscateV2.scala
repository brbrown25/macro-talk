...
class ToStringObfuscateV2(fieldsToObfuscate: Any*) extends StaticAnnotation {
  def macroTransform(annottees: Any*): Any = macro ToStringObfuscateV2Impl.impl
}

object ToStringObfuscateV2Impl {
...
//      .mkString(x.productPrefix + "(", ",", ")")
  ...
  def impl(c: whitebox.Context)(annottees: c.Expr[Any]*): c.Expr[c.universe.ClassDef] = {
    ...
    def extractAnnotationParameters(tree: Tree): List[c.universe.Tree] = tree match {
      ...
      case _ => throw new Exception("ToStringObfuscateV2 annotation must have at least one parameter.")
    }
    ...
    def modifiedDeclaration(classDecl: ClassDef): c.Expr[ClassDef] = {
      ...
      val newToString = q"""
       override def toString: ${typeOf[String]} = {
        com.bbrownsound.macros.ToStringObfuscateV2Impl._toString(this, $sensitiveFields)
       }
      """
